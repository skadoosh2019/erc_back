...
tableSchema.pre('save', function (next) {
    try {
        ...
        if (this.password)
            this.password = this.encryptPassword(this.password, this.makeSalt(parseInt(process.env.SALT_ROUNDS)));
...
tableSchema.method('getAccessTokenObject', function (platform) {
    let token = this.encryptPassword(new Date().toISOString(), this.makeSalt(parseInt(process.env.SALT_ROUNDS)));
